...
define Package/dnscrypt-proxy2/install
	...
	sed -i "s/^listen_addresses = .*/listen_addresses = ['127.0.0.1:5335']/" $(1)/etc/dnscrypt-proxy2/dnscrypt-proxy.toml
	...
	sed -i 's/^block_ipv6 = false/block_ipv6 = true/g' $(1)/etc/dnscrypt-proxy2/dnscrypt-proxy.toml
	sed -i 's/^cache_size = 4096/cache_size = 10240/g' $(1)/etc/dnscrypt-proxy2/dnscrypt-proxy.toml	
